    ...
    <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
            <div className="lg:col-span-7">
              ...
              <motion.p {...fade} transition={{ ...fade.transition, delay: 0.25 }} className="text-[1.125rem] text-navy/60 leading-relaxed max-w-[540px]">
                ...
                <a href="https://syntax.id" target="_blank" rel="noopener noreferrer" className="text-accent-dark font-semibold link-underline">Syntax Solution</a>{" "}
            ...
            <div>
              ...
              <div className="space-y-0">
                {profile.recentSkills.map((skill, i) => (
                  <motion.div key={skill} initial={{ opacity: 0, x: -8 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.04 }} className="group flex items-center gap-3 py-3 border-b border-navy/[0.04] hover:border-accent/15 transition-all duration-300">
                    <span className="w-1.5 h-1.5 bg-accent/30 group-hover:bg-accent shrink-0 transition-colors duration-300" />
                    <span className="text-[0.9375rem] text-navy/70 group-hover:text-navy font-medium transition-colors duration-300">{skill}</span>
                  </motion.div>
                ))}
              </div>
            </div>
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {profile.allSkills.map((skill, i) => (
                  <motion.span key={skill} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.3, delay: i * 0.02 }} className="text-[0.8125rem] text-navy/40 border border-navy/[0.08] px-3 py-1.5 font-medium hover:text-accent-dark hover:border-accent/25 hover:bg-accent-muted transition-all duration-300">
                    {skill}
                  </motion.span>
                ))}
              </div>
      ...
      <section className="py-20 md:py-28 bg-warm-gray/30 noise-overlay relative">
        <div className="relative z-10 max-w-[1280px] mx-auto px-6 md:px-10">
          ...
          {/* Timeline with accent vertical line */}
          <div className="relative">
            <div className="absolute left-0 md:left-[12.5%] top-0 bottom-0 w-px bg-gradient-to-b from-accent/30 via-accent/15 to-transparent hidden md:block" />
                  ...
                  <div className="py-8 md:py-10">
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-8">
                      <div className="md:col-span-3 relative">
                        {/* Timeline dot */}
                        <div className="absolute left-[-4px] md:left-[calc(50%-4px)] top-1 w-2 h-2 bg-accent hidden md:block" />
                        <h3 className="text-[1.375rem] font-bold">{exp.company}</h3>
                        {i === 0 && <span className="inline-block text-[0.6875rem] tracking-[0.1em] uppercase bg-accent text-navy px-2 py-0.5 mt-2 font-semibold">Current</span>}
                      </div>
                            ...
                            <ul className="space-y-1.5">
                              {role.summary.map((item) => (
                                <li key={item} className="flex items-start gap-3 text-[0.9375rem] text-navy/50">
                                  <span className="w-1 h-1 bg-accent/30 mt-[0.55rem] shrink-0" />
                                  {item}
                                </li>
                              ))}
                            </ul>
