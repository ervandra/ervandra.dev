  ...
  return (
    <div className="p-8 md:p-10 bg-white border border-navy/[0.08] hover:border-navy/[0.15] transition-colors duration-500">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-[0.6875rem] tracking-[0.12em] uppercase text-navy/35 font-semibold">{resource.type}</span>
      </div>
      ...
      <div className="flex flex-wrap gap-4 mb-8">
        {resource.points.map((point) => (
          <div key={point} className="flex items-center gap-2 text-[0.8125rem] text-navy/40">
            <FileCheck size={14} strokeWidth={1.5} className="text-navy/25" />{point}
          </div>
        ))}
      </div>
      {submitted ? (
        ...
        <form onSubmit={handleSubmit} className="flex gap-3">
          ...
          <button type="submit" disabled={loading} className="bg-navy text-white px-6 py-3 font-[family-name:var(--font-body)] font-semibold text-[0.8125rem] tracking-[0.04em] uppercase flex items-center gap-2 hover:bg-navy-light transition-colors duration-300 cursor-pointer shrink-0 disabled:opacity-50">
            {loading ? "..." : "Get It"}<ArrowRight size={14} />
          </button>
        </form>
      )}
    </div>
        ...
        <div className="max-w-[1280px] mx-auto px-6 md:px-10">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {resources.map((resource, i) => (
              <motion.div key={resource.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 + i * 0.08, ease: [0.16, 1, 0.3, 1] }}>
