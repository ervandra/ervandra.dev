...
function renderMarkdown(content: string) {
  // Simple markdown-to-JSX for headings, paragraphs, lists, bold, links, code
      ...
      elements.push(
        <ul key={`ul-${i}`} className="space-y-2 mb-6 ml-0">
          {items.map((item, j) => (
            ...
            >
              <span className="w-1.5 h-1.5 bg-navy/20 mt-[0.5rem] shrink-0" />
        ...
        <ol key={`ol-${i}`} className="space-y-2 mb-6 ml-0 list-none">
          {items.map((item, j) => (
            <li
              ...
              className="flex items-start gap-3 text-[1rem] text-navy/60 leading-relaxed"
            >
              <span className="text-[0.8125rem] text-navy/25 font-medium mt-0.5 shrink-0 w-5">
                {j + 1}.
              </span>
              <span>{formatInline(item)}</span>
            </li>
    ...
    if (first.type === "bold") {
      ...
      parts.push(
        <a
          ...
          className="text-navy font-medium link-underline"
        >
          {first.match[1]}
        </a>
      );
      ...
      parts.push(
        <code
          key={keyIdx++}
          className="text-[0.9em] bg-navy/[0.04] px-1.5 py-0.5 font-mono text-navy/70"
        >
          {first.match[1]}
        </code>
      );
  ...
  return (
    <>
      ...
      <section className="pt-32 pb-12 md:pt-40 md:pb-16">
        <div className="max-w-[720px] mx-auto px-6 md:px-10">
          ...
          >
            <Link
              href="/blog"
              className="inline-flex items-center gap-2 text-[0.8125rem] text-navy/40 hover:text-navy transition-colors duration-300 mb-8"
            >
              <ArrowLeft size={14} />
              Back to Blog
            </Link>
      ...
      <section className="py-16 md:py-20">
        <div className="max-w-[720px] mx-auto px-6 md:px-10">
          <div className="p-8 md:p-10 bg-warm-gray/50 border border-navy/[0.06]">
